...
void OttoMin::init(int YL, int YR, int RL, int RR, bool load_calibration, int Buzzer) {
...
///////////////////////////////////////////////////////////////////
void OttoMin::_moveServos(int time, int  servo_target[]) {
...
void OttoMin::_moveSingle(int position, int servo_number) {
...
void OttoMin::oscillateServos(int A[4], int O[4], int T, double phase_diff[4], float cycle=1){
...
void OttoMin::_execute(int A[4], int O[4], int T, double phase_diff[4], float steps = 1.0){
...
bool OttoMin::getRestState(){
    return isOttoResting;
}

void OttoMin::setRestState(bool state){
...
//--    T: Period
//---------------------------------------------------------
void OttoMin::jump(float steps, int T){
...
//---------------------------------------------------------
void OttoMin::walk(float steps, int T, int dir){
...
//---------------------------------------------------------
void OttoMin::turn(float steps, int T, int dir){

  //-- Same coordination than for walking (see OttoMin::walk)
...
//---------------------------------------------------------
void OttoMin::bend (int steps, int T, int dir){
...
//---------------------------------------------------------
void OttoMin::shakeLeg (int steps,int T,int dir){
...
//---------------------------------------------------------
void OttoMin::updown(float steps, int T, int h){
...
//---------------------------------------------------------
void OttoMin::swing(float steps, int T, int h){
...
//--     h : Amount of swing (from 0 to 50 aprox)
//---------------------------------------------------------
void OttoMin::tiptoeSwing(float steps, int T, int h){
...
//---------------------------------------------------------
void OttoMin::jitter(float steps, int T, int h){
...
//---------------------------------------------------------
void OttoMin::ascendingTurn(float steps, int T, int h){
...
//--    dir: Direction: LEFT / RIGHT
//---------------------------------------------------------
void OttoMin::moonwalker(float steps, int T, int h, int dir){
...
//--     dir:  Direction: LEFT / RIGHT
//-----------------------------------------------------------
void OttoMin::crusaito(float steps, int T, int h, int dir){
...
//---------------------------------------------------------
void OttoMin::flapping(float steps, int T, int h, int dir){
...
void OttoMin::_tone (float noteFrequency, long noteDuration, int silentDuration){
    ...
    // delay(500);
      ...
      tone(OttoMin::pinBuzzer, noteFrequency, noteDuration);
      delay(noteDuration);       //milliseconds to microseconds
      //noTone(PIN_Buzzer);
      delay(silentDuration);
}

void OttoMin::bendTones (float initFrequency, float finalFrequency, float prop, long noteDuration, int silentDuration){
...
void OttoMin::sing(int songName){
  switch(songName){
...
void OttoMin::playGesture(int gesture){
...
void OttoMin::enableServoLimit(int diff_limit) {
  for (int i = 0; i < 4; i++) {
    servo[i].SetLimiter(diff_limit);
  }
}

void OttoMin::disableServoLimit() {
  for (int i = 0; i < 4; i++) {
